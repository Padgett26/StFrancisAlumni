...
if (isset ( $_FILES ['image'] ['name'] )) {
	$folder = "pics/yearbooks/$useryear";
...
if (filter_input ( INPUT_POST, 'editybook', FILTER_SANITIZE_NUMBER_INT )) {
	...
	if ($rename) {
		$old = "/home/alumni/public_html/pics/yearbooks/$useryear/$pic.jpg";
		$new = "/home/alumni/public_html/pics/yearbooks/$useryear/$rename.jpg";
		rename ( $old, $new ) or die ();
	}
	if ($delpic) {
		$file = "/home/alumni/public_html/pics/yearbooks/$useryear/$delpic";
		unlink ( $file ) or die ();
	}
}

if (! is_dir ( "pics/yearbooks/$useryear" )) {
	mkdir ( "pics/yearbooks/$useryear", 0777, true );
}
echo "Add a yearbook page:<br />";
echo "<form method='post' action='index.php?page=modadmin&choice=ybook' enctype='multipart/form-data'>Number the page, do not use a number already being used by another page, it will overwrite it.<br />1-999 <input type='text' name='picname' size='3' maxlength='3' /><br /><input type='file' name='image' size='20' /><br /><input type='submit' value=' -Upload- ' /></form>";
echo "Currently loaded yearbook pictures:<br />";
$pages = array ();
foreach ( new DirectoryIterator ( "pics/yearbooks/$useryear" ) as $j ) {
	if (! $j->isDot ()) {
		$pages [] = "$j";
	}
}
...
foreach ( $pages as $j ) {
	...
	$name = $match [0];
	echo <<<_END
	    <form method='post' action='index.php?page=modadmin&choice=ybook' enctype='multipart/form-data'>
	    <div style='border:1px solid #000000; padding:5px; margin:5px; float:left;'>
	        <img src='pics/yearbooks/$useryear/$j' style='max-width:250px; margin:10px auto;' alt='' />

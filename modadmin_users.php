...
if (filter_input ( INPUT_POST, 'changeThen', FILTER_SANITIZE_NUMBER_INT ))
	$changeThen = filter_input ( INPUT_POST, 'echangeThen', FILTER_SANITIZE_NUMBER_INT );
...
if (filter_input ( INPUT_POST, 'userid', FILTER_SANITIZE_STRING )) {
	...
	if ($emailold != $emailup) {
		$substmt = $db_conn->prepare ( "UPDATE users SET bademail='0' WHERE id=?" );
	...
	if ($userlvlup == "2") {
		...
		if ($returnlvl != "2") {
			...
			$stmt = $db_conn->prepare ( "INSERT INTO bboard VALUES" . "(NULL, ?, ?, '1', ?,'0','1','0','0')" );
			$stmt->execute ( array (
					$yearup,
					$createdup,
					$msgmodcheck
	...
	if ($useridup == "new") {
		if ($firstnameup == "" || $lastnameup == "")
			...
			$stmt = $db_conn->query ( "SELECT COUNT(id) FROM users WHERE user='$userup'" );
			...
			if ($initnum1 >= 1) {
				...
				$substmt = $db_conn->prepare ( "SELECT id FROM users WHERE firstname=?,miname=?,lastname=?,year=?,deceased=? ORDER BY id DESC LIMIT 1" );
				$substmt->execute ( array (
						$firstnameup,
						$minameup,
						$lastnameup,
		...
		if (($userup == "" && $usercreatedup == "1") || ($emailup == "" && $usercreatedup == "1") || $firstnameup == "" || $lastnameup == "")
			...
			if ($userup != $userprofup) {
				...
				if ($initnum1 >= 1) {
					...
					$stmt = $db_conn->prepare ( "UPDATE users SET user=?, firstname=?, miname=?, lastname=?, maidenname=?, address=?, cityst=?, zip=?, email=?, phone=?, userlvl=?, year=?, approved=?, descpara=?, deceased=?, usercreated=? WHERE id=?" );
					$stmt->execute ( array (
							$userup,
							$firstnameup,
							$minameup,
					...
					$stmt = $db_conn->prepare ( "UPDATE usersettings SET notmsg=?, notbb=?, notassoc=? WHERE userid=?" );
					$stmt->execute ( array (
							$notmsg,
							$notbb,
							$notassoc,
				...
				$stmt = $db_conn->prepare ( "UPDATE users SET firstname=?, miname=?, lastname=?, maidenname=?, address=?, cityst=?, zip=?, email=?, phone=?, userlvl=?, year=?, approved=?, descpara=?, deceased=?, usercreated=? WHERE id=?" );
				$stmt->execute ( array (
						$firstnameup,
						$minameup,
						$lastnameup,
...
if (filter_input ( INPUT_POST, 'userselect', FILTER_SANITIZE_STRING )) {
	$idgetuser = filter_input ( INPUT_POST, 'userselect', FILTER_SANITIZE_STRING );
	...
	echo <<<_END
	<form method="post" action="index.php?page=modadmin&choice=users" enctype='multipart/form-data'><div style='font-size:1em; text-align:center;'>Has this user forrgotten his/her password?<br />Click this button to send a password reset email.<br /><input type='hidden' name='pwdreset' value='$idgetuser' /><input type='hidden' name='pwdresetemail' value='$emailgetuser' /><input type='submit' value='Password Reset' style='border-radius:8px; -moz-border-radius:8px;' /></div></form><br /><br />
	<div style='font-size:1em; text-align:center;'>Items marked with an * are required</div>
	<form method="post" action="index.php?page=modadmin&choice=users" enctype='multipart/form-data'>
	...
	if ($ucreatedgetuser == "1") {
		...
		if ($userlvlgetuser == '2')
			echo " selected='selected'";
		echo ">2 - Moderator</option></select>";
	...
	echo <<<_END
	        <tr>
	            <td>User year:</td>
	            <td><select name="yearcontact" size="1" rel='none'><option value="9999">Faculty</option>
	_END;
	$getyear = date ( 'Y' );
	$firstyear = "1900";
...
$selectuseryear = filter_input ( INPUT_POST, 'selectuseryear', FILTER_SANITIZE_NUMBER_INT );
$stmt = $db_conn->prepare ( "SELECT id, firstname, miname, lastname FROM users WHERE year=? ORDER BY lastname" );
$stmt->execute ( array (
		$selectuseryear
) );
echo "Please select a user to edit:<br /><form method='post' action='index.php?page=modadmin&choice=users'><select name='userselect' size='1'><option value='new'>Create new user</option>";

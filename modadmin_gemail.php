...
if (filter_input(INPUT_POST, 'sendgemail', FILTER_SANITIZE_NUMBER_INT)) {
    ...
    $stmt = $db_conn->prepare("SELECT t1.email, t1.firstname, t1.miname, t1.lastname FROM users AS t1 INNER JOIN usersettings AS t2 ON t2.userid=t1.id WHERE t2.notassoc='1' AND t1.year=?");
...
echo "This will send an email to every $useryear user that has email notifications allowed:<br /><br />
To [user name]:<br />
<form method='post' action='index.php?page=modadmin&choice=gemail'>
<textarea cols='80' rows='10' name='gemail'></textarea><br /><br />
Thank you,<br />
St Francis Kansas Alumni Website Administration<br /><br />
<input type='hidden' name='sendgemail' value='1' />
